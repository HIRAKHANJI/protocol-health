// plans/tempcut.js — TEMP CUT v3.1: "THE DUBAI 13" (Aug 3 → Aug 14, weigh-in Sat Aug 15).
// Owner-designed pre-flight cut. v3 spec finalised 2026-08-02; v3.1 recalendar
// 2026-08-03 to match the owner's ACTUAL fast pattern (reported day 1):
// fast days are WED / SAT / SUN — each fast runs from 6PM the evening before
// through the fast day, breaking at 9AM the morning after (single Wed fast
// ≈ 39h: Tue 6PM → Thu 9AM · weekend double ≈ 63h: Fri 6PM → Mon 9AM).
// Eating days (Mon/Tue/Thu/Fri): ~900 cal in a 9AM–3PM window (protein powder
// + 2 clean bars + rice cakes + carrots/pickles — 100g protein floor) + one
// ~100-min A/B/C/D session (A=Mon chest+biceps+weighted abs · B=Tue back+whale
// lower back+forearms · C=Thu legs+side-abs+gut-tilt fix · D=Fri shoulders+
// arms+steel core) + 10-min burst. Fast days: Sat light @75%, Sun WALK ONLY
// (law), Wed burst + walk. Gates: Thu Aug 6 ≤98.2 · Mon Aug 10 ≤95.0 (both
// weighed at the end of a fast, before breaking it). Low-residue from Thu 13
// night, FLUSH Fri 14 (~750, powder + rice cakes), weigh-in Sat 15 → flight.
// D3+K2 is OUT of the stack (owner call). Creatine paused until Aug 15.
...
export const tempcut = {
    ...
    badgeClass: 'agro',
    descClass: 'agro-desc',
    subtitle: 'Dubai 13. Wed/Sat/Sun fasts. 900-cal window days. Train Mon/Tue/Thu/Fri. 90.0 by the flight.',
    bannerColor: '#ff5566',
    ...
    // DOW view — every weekday has ONE fixed role, both weeks:
    // Mon = eat + SESSION A (Aug 10: GATE 2 first) · Tue = eat + SESSION B,
    // fast begins 6PM · Wed = FAST (burst + walk) · Thu = eat + SESSION C
    // (Aug 6: GATE 1 first; Aug 13: low-residue from tonight) · Fri = eat +
    // SESSION D (Aug 14: FLUSH instead) · Sat = FAST day 1, light @75% ·
    // Sun = FAST day 2 (deepest), WALK ONLY — law.
    ...
    morningSub: {
      ...
      1:'MONDAY — Eating day (Aug 3 · Aug 10). Aug 10: WEIGH FIRST — GATE 2, scale must read ≤95.0 — then break the fast at 9AM: powder in water + Osteocare, gentle. Vacuums + weigh.',
      2:'TUESDAY — Eating day (Aug 4 · Aug 11). Window 9AM–3PM. Tonight 6PM the Wednesday fast begins. Vacuums + weigh.',
      3:'WEDNESDAY — FAST (Aug 5 · Aug 12; started Tue 6PM, breaks Thu 9AM). Salt ×3 + K-tabs. Vacuums + weigh.',
      4:'THURSDAY — Eating day (Aug 6 · Aug 13). Aug 6: WEIGH FIRST — GATE 1, ≤98.2 — then break fast at 9AM. Aug 13: low-residue starts tonight (bars out). Vacuums.',
      5:'FRIDAY — Aug 7: eating day; tonight 6PM the weekend double begins (Fri 6PM → Mon 9AM) · Aug 14: FLUSH day ~750 cal, powder + rice cakes ONLY, bed by 10. Vacuums + weigh.',
      6:'SATURDAY — Aug 8: FAST day 1 of the double · Aug 15: WEIGH-IN (wake → bathroom → scale → write it down) → real breakfast → flight → creatine begins.'
    },
    eveningSub: {
      0:'SUNDAY — Nothing beyond the walk: 40-60 min easy + hip stretches + vacuums. Tomorrow 9AM the double ends — weigh before you break it (Aug 10 = GATE 2).',
      1:'MONDAY — SESSION A: chest + biceps + weighted abs + burst (~100 min). Card is open on the WORKOUTS tab.',
      2:'TUESDAY — SESSION B: back + whale lower back + forearms + burst. Last bite was 3PM; fast is on from 6PM.',
      3:'WEDNESDAY — Fasted: 10-min burst + 40-60 min walk + vacuums. Optional light circuit @75%, nothing near failure. No grinding.',
      4:'THURSDAY — SESSION C: legs + side-abs + gut-tilt fix + burst. Aug 13: bars OUT from tonight (low-residue).',
      5:'FRIDAY — Aug 7: SESSION D — shoulders + arms + steel core, neck ×2 today · Aug 14: FLUSH — smooth circuit ×3, NO new soreness allowed.',
      6:'SATURDAY — Aug 8: light full-body @75% + 30-40 min walk (fast day 1) · Aug 15: rest. The scale has spoken, the flight is boarding.'
    },
    stretchSub: {
      0:'DEEP-FAST day: full hip session — 90/90 · frog · pigeon · cossack · deep squat + hip-flexor stretch + neck isometrics (20 min).',
      1:'Doorframe chest · shoulder dislocates · wrist extension + hip-flexor couch stretch 2×60s/side (10 min).',
      ...
      6:'Hips gentle: 90/90 · pigeon · cossack + hip-flexor stretch (10 min).'
    },
    ...
    checklistNormal: [
      ...
      { id:'m3', group:'MORNING', label:'Morning role done (see WORKOUTS + today’s sub-text)', sub:'Eating days: weigh + vacuums + start the sneaky drips — the one big session is in the evening. Gate mornings: weigh BEFORE breaking the fast.' },
      ...
      { id:'e2', group:'EVENING', label:'Evening session done (A Mon · B Tue · C Thu · D Fri — see WORKOUTS)', sub:'~100 min incl. the 10-min burst + neck 5 min. Non-skippable — it carries ALL the muscle retention at 100g protein.' },
    ...
    // (Fri 6PM → Mon 9AM). Saturdays train light @75%, Sundays are WALK ONLY
    // (law), Wednesdays are burst + walk.
    checklistFast: [
      ...
      { id:'wf4', group:'FAST', label:'Training per fast-day spec — capped, nothing near failure', sub:'Sat: light full-body @75% + walk · Sun: WALK ONLY, 40-60 min — law · Wed: 10-min burst + walk (+ optional light circuit @75%).' },
    ...
    workoutContent() {
      ...
      const _todayByDow = {
        0: { label:'SUNDAY — FAST DAY 2, THE DEEP ONE', what:'WALK ONLY — 40-60 min easy + hips + vacuums. NO lifting at 48h+ fasted, by law. Tomorrow 9AM the double ends — Aug 10: weigh FIRST (GATE 2 ≤95.0), then break it.' },
        1: { label:'MONDAY — EATING DAY (900 · 9AM-3PM)', what:'SESSION A tonight: chest + biceps + weighted abs + burst. Card is open below. Aug 10: GATE 2 weigh-in before breaking the fast at 9AM.' },
        2: { label:'TUESDAY — EATING DAY (900 · 9AM-3PM)', what:'SESSION B tonight: back + whale lower back + forearms + burst. Card is open below. The Wednesday fast begins 6PM tonight — last bite was 3PM anyway.' },
        3: { label:'WEDNESDAY — FAST DAY (~39H)', what:'Started 6PM yesterday, breaks tomorrow 9AM. Today: 10-min burst + 40-60 min walk + vacuums; optional light circuit @75%, nothing near failure. Your card: WEDNESDAY FAST, open below.' },
        4: { label:'THURSDAY — EATING DAY (900 · 9AM-3PM)', what:'Break the fast 9AM (Aug 6: weigh FIRST — GATE 1 ≤98.2). SESSION C tonight: legs + side-abs + gut-tilt fix + burst. Card is open below. Aug 13: bars OUT from tonight.' },
        5: { label:'FRIDAY', what:'Aug 7: eating day — SESSION D tonight (shoulders + arms + steel core, neck ×2), weekend double begins 6PM · Aug 14: FLUSH — smooth circuit ×3 only, ~750 cal, bed by 10.' },
        6: { label:'SATURDAY', what:'Aug 8: FAST day 1 of the double — light full-body @75% + 30-40 min walk (card below) · Aug 15: WEIGH-IN morning → flight → creatine. The block is over.' }
      };
      ...
      <p class="section-note">Mon Aug 3 → Fri Aug 14 · WEIGH-IN Sat Aug 15 morning, then the flight. Train Mon/Tue/Thu/Fri (~100-min evening session + burst), fast Wed/Sat/Sun — every fast runs 6PM the night before → 9AM the morning after. Every exercise explained in plain English below.</p>

      <div class="rule-card" style="border-left-color:#ff5566">
        <div class="rule-num">THE CALENDAR — DATES ARE LAW</div>
        <div class="rule-text">Mon 3 eat+A · Tue 4 eat+B (fast from 6PM) · Wed 5 FAST · Thu 6 eat+C ✓GATE 1 ≤98.2 at 9AM · Fri 7 eat+D (double from 6PM) · Sat 8 FAST light · Sun 9 FAST walk-only · Mon 10 eat+A ✓GATE 2 ≤95.0 at 9AM · Tue 11 eat+B (fast from 6PM) · Wed 12 FAST · Thu 13 eat+C, low-residue from tonight · Fri 14 FLUSH ~750 · Sat 15 WEIGH-IN → flight → creatine</div>
        <div class="rule-sub">Eating days ~900 cal, window 9AM–3PM. Fasts: Wed singles ≈39h (Tue 6PM → Thu 9AM) · weekend double ≈63h (Fri 6PM → Mon 9AM). Both gates are weighed at the END of a fast, before breaking it — that’s your truest scale of the week. Every extra burst = +130 toward 90.0.</div>
      </div>
      ...
      <div class="section-title">THE ROTATION <span>— A MON · B TUE · C THU · D FRI</span></div>

      ${workoutCard('SESSION A — CHEST + BICEPS + WEIGHTED ABS','MONDAYS — AUG 3 · AUG 10',
      ...
      ${workoutCard('SESSION B — BACK + WHALE LOWER BACK + FOREARMS','TUESDAYS — AUG 4 · AUG 11',
        ...
        exRow('BURST + neck 5 min','The Wednesday fast starts 6PM tonight — session runs as normal, you’re fueled from the window.','10 min'),
        stretchRow('Full shoulder sequence + hip-flexor stretch + neck isometrics','','12 min'),
        'TUE'
      )}

      ${workoutCard('SESSION C — LEGS + SIDE-ABS + THE GUT-TILT FIX','THURSDAYS — AUG 6 · AUG 13',
      ...
      ${workoutCard('SESSION D — SHOULDERS + ARMS RD 2 + STEEL CORE','FRIDAY AUG 7 (AUG 14 = FLUSH INSTEAD)',
        ...
        exRow('NECK ROUTINE ×2 TODAY + BURST','Full 5-min neck protocol morning AND evening — growth needs frequency. The weekend double starts 6PM tonight.','2×5 + 10 min'),
        stretchRow('Doorframe chest + rear shoulder + wrists + hip-flexor stretch','','8 min'),
        'FRI'
      )}

      <div class="section-title">THE FAST DAYS <span>+ THE FINISH</span></div>

      ${workoutCard('WEDNESDAY FAST — AUG 5 · AUG 12 (~39H: TUE 6PM → THU 9AM)','BURST + WALK · OPTIONAL LIGHT CIRCUIT',
        exRow('10-min burst','Pick from the menu — low-skill cardio is fine fasted. Nothing maximal.','10 min')+
        exRow('Walk','40-60 min easy, any time of day.','40-60 min')+
        exRow('Optional light circuit @75%','Only if you feel strong: chest press ×12 · pulldown ×12 · goblet ×12 — 2 smooth rounds, nothing near failure.','optional')+
        exRow('Vacuums + neck protocol','3×20-30s + 5 min.','10 min')+
        exRow('Salt ×3 + both K-tabs','Electrolytes are the whole game.','all day'),
        stretchRow('Gentle full-body flow','Nothing aggressive.','10 min'),
        'WED'
      )}

      ${workoutCard('SATURDAY FAST — AUG 8 (DAY 1 OF THE DOUBLE)','LIGHT FULL-BODY @75% + WALK',
        exRow('Light circuit ×3, smooth','Goblet squat ×12 · pulldown (light) ×12 · push-ups ×10 · suitcase carry 40 steps/side — zero grinding, 2 in the tank everywhere.','3 rounds')+
        exRow('Walk','30-40 min easy.','30-40 min')+
        exRow('Vacuums + neck + salt ×3 + K-tabs','The double runs Fri 6PM → Mon 9AM. Today is the manageable half.','all day'),
        stretchRow('Hips gentle + hip-flexor stretch','','10 min'),
        'SAT'
      )}

      ${workoutCard('SUNDAY FAST — AUG 9 (DAY 2, THE DEEP ONE)','WALK ONLY — LAW',
    ...
    nutritionContent(s) {
      ...
      <p class="section-note">900 cal in a 9AM–3PM window on eating days (Mon/Tue/Thu/Fri). 100g protein is the daily floor and the whole muscle-retention vote. Set Settings ceiling to 900.</p>
      ...
      ${ruleCard('BMR ~2,030→1,930 · STANDARD TDEE ~2,430→2,310','Mifflin-St Jeor at 24M · 180cm, recalculated as you shrink, metabolism-slowdown included. Standard = what you burn by being alive.','Deliberate burn per day: session ~650-1,000 + burst ~130 + walk ~90 = +1,050-1,200 on training days · fast days lighter by design. Daily deficit: eating days ~2,300-2,800 · fast days ~2,700-3,200.','#82e0aa')}
      ${ruleCard('THE LEDGER — AUG 3 → 14 (4 FASTS + 7 EATING DAYS + FLUSH)','OUT ~38,000-41,500 base (more with extra bursts) · IN ~7,050 · NET ~31,000-34,500 base.','Two fewer zero-days than the original 6-fast sketch, so the extras column matters MORE now: base lands ~91.5-92.5 from ~99-100 · with extra bursts most days + both gates hit + a clean finish, ~90.5-91.5 and 90.0 stays reachable. Rebound after +2-3kg = glycogen refilling, not fat — pre-accepted.','#82e0aa')}

      <div class="section-title" style="margin-top:8px">SUPPLEMENT <span>CLOCK — D3+K2 IS OUT</span></div>
      ${ruleCard('FAST DAYS (WED 5 · SAT 8 · SUN 9 · WED 12)','Wake: MCT gel · zinc if Mon/Wed/Fri · K-tab mid-AM + mid-PM · salt ⅓ tsp ×3 (×4 Sundays) · magnesium at bed.','NO Osteocare — calcium needs food. Preworkout is your throttle; the STOP rule is doubly live fasted.','#82e0aa')}
    ...
    rulesContent(s) {
      ...
      ${ruleCard('RULE 02','STOP cluster: palpitations · chest tightness · vision narrowing','Salt water + 50g carbs IMMEDIATELY. Fast over if fasting. Zero shame, total obedience. Doubly live deep into the weekend double and with preworkout on board.')}
      ...
      ${ruleCard('RULE 04','Deep Sundays = walk only. Law.','48h+ into the double: fuel tank empty, cortisol peaked, injury risk stupid. Light movement spares muscle better than grinding. Saturdays cap at 75%, Wednesdays are burst + walk.')}
      ...
      ${ruleCard('THE LEVERS — PICK ONE AT THE GATE, NOT BEFORE','(a) +2 extra bursts daily — the simplest · (b) eating days drop to 800 (cut one bar) · (c) extend the Wed Aug 12 fast through Friday 9AM (skip Thu eating, C moves to Fri).','One lever, executed hot, beats three levers planned cold.','var(--accent2)')}
      ...
      ${ruleCard('SAT AUG 15 MORNING','Wake → bathroom → scale → write it down → real breakfast → flight → creatine 5g begins.','With extra bursts most days + both gates + a clean finish: ~90.5-91.5, and 90.0 in reach. Rebound +2-3kg in the days after = glycogen refilling, not fat. Pre-accepted.','#70c8ff')}
